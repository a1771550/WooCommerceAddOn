...
namespace WooCommerceAddOn
{
	public partial class frmList : Form
	{
		...
		private HashSet<int> CurrentItemRecordIds { get; set; }
		...
		private async void frmList_Load(object sender, EventArgs e)
		{
			...
			switch (type)
			{
				case DataType.MyobCustomer:
					...
					break;
				case DataType.MyobProduct:
					CurrentItemRecordIds = MyobItemEditModel.GetCurrentItemIds(comInfo.AccountProfileId);
					MyobProducts = MYOBHelper.GetItemList(abss, string.Join(",", CurrentItemRecordIds));
					progressBar1.Visible = false;
		...
		private async void btnSaveDB_Click(object sender, EventArgs e)
		{
			...
			{
				case DataType.MyobCustomer:
					HashSet<int> currentCustomerIdsInDB = await MyobCustomerEditModel.GetAllIdsInDb(comInfo.AccountProfileId);

					#region add data:
					var filteredCustomers = MyobCustomers.Where(x => !currentCustomerIdsInDB.Contains(x.CustomerID)).ToList();
					cbok = await MyobCustomerEditModel.AddList(filteredCustomers, comInfo.AccountProfileId);
					...
					break;
				case DataType.MyobProduct:
					HashSet<int> currentItemIdsInDB = await MyobItemEditModel.GetAllIdsInDb(comInfo.AccountProfileId);

					#region add data:
					var filteredProducts = MyobProducts.Where(x => !currentItemIdsInDB.Contains(x.ItemID)).ToList();
					bok = await MyobItemEditModel.AddList(filteredProducts, comInfo.AccountProfileId);
					if (bok)
					{
						progressBar1.Visible = false;
				...
				case DataType.Order:
					...
					if (ModelHelper.CheckIfHasWooCustomersItems())
					{
						...
						else
						{
							...
							progressBar1.Visible = true;
							progressBar1.Style = ProgressBarStyle.Marquee;
							var currentSalesIds = SalesEditModel.GetSalesIdList();
